    ...
    if await is_active_chat(chat_id):
        if await is_music_playing(chat_id):
            ...
            await CallbackQuery.edit_message_text(
                "⏸ music playback has paused", reply_markup=play_keyboard
            )
...
async def resumevc(_, CallbackQuery):  
    ...
    if await is_active_chat(chat_id):
        if await is_music_playing(chat_id):
            ...
            rpk = f"[{user_name}](tg://user?id={str(user_id)})"
            await CallbackQuery.edit_message_text(
                "▶ music playback has resumed", reply_markup=play_keyboard
            )
...
async def G_list(_, CallbackQuery):
    ...
    if not _playlist:
        return await CallbackQuery.answer(
            "❌ This Group has no playlist in database, try to adding music into playlist.",
            show_alert=True,
        )
